...
class TaskManager {
    ...
    renderTaskContent(task) {
        // Este método ya no se usa en las tarjetas compactas
    ...
    focusTask(taskId) {
        // Marcar esta tarea como activa para el pegado de imágenes
        this.activeTaskId = taskId;
        
        // Actualizar el método getActiveTask para usar esta información
        this.getActiveTask = () => {
    ...
    showTaskDetail(taskId) {
        ...
        // Actualizar eventos para el contenido editable
    ...
    setupTaskDetailEvents(task) {
        // Configurar eventos para las textareas del modal
        const textareas = document.querySelectorAll('#taskDetailContent textarea');
        textareas.forEach(textarea => {
            textarea.addEventListener('input', (e) => {
                const contentId = parseInt(e.target.getAttribute('data-content-id'));
                if (contentId) {
                    this.updateContentInTask(task.id, contentId, e.target.value);
                }
            });
        });
    }
    ...
    viewImageFullscreen(imageSrc) {
        // Crear overlay para vista en pantalla completa
        const overlay = document.createElement('div');
        overlay.style.cssText = `
            position: fixed;
            top: 0;
